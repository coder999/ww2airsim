...
import { AIRCRAFT_CONTENT_PATH, finestFetchedLevelFor, INTERIM_ASSET_QUALITY_TIER, terrainLevelPath, TITLE_ART_BYTES, TITLE_ART_PATH, SHAPE_NOISE_PATH, DETAIL_NOISE_PATH, WEATHER_MAP_PATH, WILDCAT_MODEL_PATH } from '../../src/render/content.js'
...
describe('the built artifact', () => {
  it('ships the content it fetches at runtime, and the licence notice that must travel with it', async () => {
    ...
    try {
      ...
      expect(existsSync(join(outDir, WEATHER_MAP_PATH)), WEATHER_MAP_PATH).toBe(true)
