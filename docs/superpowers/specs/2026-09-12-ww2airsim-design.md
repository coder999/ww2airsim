...
| Effects realism | any | Photoreal explosions and water splashes: bomb, rocket, shell and crash detonations over land and water (fireball, smoke column, debris; splash plume, spray and foam ring) | §4 | Not started; requested by Mark 2026-09-26. Today every detonation and crash is one camera-facing quad that grows and fades (`src/render/scene/impactEffect.ts`; ordnance impacts reuse its curve in `src/render/ordnance.ts`), verified 2026-09-26 by reading both. Needs its own design |
| Ordnance models | any | Source real 3D models for bombs, rockets and other stores (drop tanks, HVAR rails, torpedoes later) to replace the primitive meshes | §4, §9, §10 | Not started; requested by Mark 2026-09-26. Bombs and rockets are box and cylinder `InstancedMesh` pools in `src/render/ordnance.ts` today. `ASSETS.md`'s candidate table lists one bomb (`bomb-m64-500lb.glb`, CC-BY 4.0, not shipped) and no rocket; its only torpedo is a 1904 ship torpedo, not the aerial Mk 13. Ships through Z1's `models:build` pipeline and the Hangar |
