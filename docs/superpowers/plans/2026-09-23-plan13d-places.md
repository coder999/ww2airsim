...
- **No network at runtime, and none at build time either once cached** — the
  ...
  ONCE into `tools/scenery/cache/`, committed, and the build reads only the
  cache from then on, exactly like `binahaan.json`/`daguitan.json` today.
...
**Files:**
- Create: `tools/scenery/cache/places-overpass.json` (committed, fetched
  once by this task)
...
```bash
git add tools/scenery/cache/places-overpass.json tools/scenery/build.ts \
