...
Near the cloud field creation (after `cloudTier` is decided, before `createTerrainMesh`):

```ts
  // Plan 16c: the scenario's hour, or the DEV override. Apparent solar time.
  const forcedTimeOfDay = import.meta.env.DEV ? timeOfDayFromQuery(location.search) : undefined
  const scenarioTimeOfDay = forcedTimeOfDay ?? bundle.scenario.weather.timeOfDay ?? DEFAULT_TIME_OF_DAY
  let sunState = { timeOfDay: scenarioTimeOfDay, elevationDeg: 90, azimuthDeg: 180, direction: { x: 0, y: 1, z: 0 } }
```
...
test('the same view at 06:30, 12:00 and 17:00: warm ends, blue noon, brightest ground at noon, geometry as computed', async ({ page }) => {
  ...
  for (const h of hours) {
    ...
    const expected = sunPosition(TERRAIN_HEADER.centreLatDeg, h)
