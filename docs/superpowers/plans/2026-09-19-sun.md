...
- [x] **Step 2: Run** `npx vitest run tests/sim/scenario.test.ts` → FAIL (`timeOfDay` unknown key under `.strict()`).

- [x] **Step 3: Implement.** In `src/sim/scenario.ts` inside the `weather` object, after the `clouds` field:
...
- [x] **Step 4: Run** `npx vitest run tests/sim/scenario.test.ts` → PASS.
...
- [x] **Step 1: Write the failing test** `tests/render/sun.test.ts`:
...
- [x] **Step 2: Run** `npx vitest run tests/render/sun.test.ts` → FAIL (module not found).

- [x] **Step 3: Implement** `src/render/sky/sun.ts`:
...
- [x] **Step 4: Run** `npx vitest run tests/render/sun.test.ts` → PASS.
...
- [x] **Step 1: Write the failing tests.** `tests/render/palette.test.ts`:
...
- [x] **Step 2: Run** `npx vitest run tests/render/palette.test.ts tests/render/scene.test.ts` → FAIL.

- [x] **Step 3: Implement** `src/render/sky/palette.ts`:
...
- [x] **Step 4: Run** `npx vitest run tests/render/palette.test.ts tests/render/scene.test.ts tests/render/clouds.test.ts tests/render/cloudShadow.test.ts` → PASS (the last two prove the constant move broke no importer).

- [x] **Step 5: Run** `npm run verify; echo rc=$?` → `rc=0`. depcruise: `scene/sky.ts -> sky/palette.ts` and `scene/lighting.ts -> sky/palette.ts` are new edges; `palette.ts` imports nothing, so no cycle.
...
- [x] **Step 1: Write the failing test** (append inside `describe('cloud shadow map (Plan 16b)'` in `tests/render/cloudShadow.test.ts`):
...
- [x] **Step 2: Run** `npx vitest run tests/render/cloudShadow.test.ts` → FAIL.

- [x] **Step 3: Implement**, one consumer at a time, running `npx tsc --noEmit` after each.
...
- [x] **Step 1: Imports and boot.** In `main.ts` add:
...
- [x] **Step 2: The frame.** Right after `const skyTimeS = ...` (line ~1164), before `clouds.update(...)`:
...
- [x] **Step 3: Diagnostics.** In the `__ww2` block after `cloudShadowAt`:
...
- [x] **Step 4: Run** `npm run verify; echo rc=$?` → `rc=0`. Then the GPU smoke: `npm run test:tier2 -- tests/e2e/clouds.spec.ts tests/e2e/cloudShadow.spec.ts` with the PW env. Expect: the clouds spec green; in `cloudShadow.spec.ts` the **carrier-deck** case may now fail, because deck-quals runs at 16:30 and the new azimuth may have moved the cloud off the carrier at tick 0. If it does: probe the deck strip's off/on means at `&timeOfDay=` 10, 12, 14 and 16.5 with a throwaway spec (copy the case, loop the hours, print), pick the hour with the largest darkening, pin it in the case's URL with a comment naming the measured values, and delete the probe. READ `clouds-chocks-up.png`: the free-flight sun is now 10:00, 53° up in the south-east; the deck's lit sides face it.
...
- [x] **Step 1: Write `tests/e2e/sun.spec.ts`**:
