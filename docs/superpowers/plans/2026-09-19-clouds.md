...
- [x] **Step 2: Run** `npx vitest run tests/sim/scenario.test.ts` → FAIL (`clouds` unknown key).

- [x] **Step 3: Extend the schema** in `src/sim/scenario.ts`. Above `ScenarioObject`:
...
- [x] **Step 4: Add the decks.** `content/scenarios/free-flight.json` weather:
...
- [x] **Step 5: Run** the scenario tests → PASS. `npm run verify` → rc 0. Commit:
...
- [x] **Step 1: Write the failing generator tests** `tests/tools/skyNoise.test.ts`:
...
- [x] **Step 2: Write `src/render/sky/noise.ts`** (browser-safe constants only):
...
- [x] **Step 3: Write `tools/sky/noise.ts`**:
...
- [x] **Step 4: Write `tools/sky/load.ts` and `tools/sky/build.ts`**:
...
- [x] **Step 5: Build and commit the volumes**: `npm run sky:build` (expect tens of seconds), then `sha256sum content/sky/*.gz`.

- [x] **Step 6: Paste the digests** into `COMMITTED_SHA256` in the test. Write `content/sky/NOTICE.md`:
...
- [x] **Step 7: Content paths and the browser loader.** In `src/render/content.ts`:
...
- [x] **Step 8: Run** `npx vitest run tests/tools/skyNoise.test.ts tests/render/skyLoad.test.ts tests/build/dist.test.ts` → PASS. `npm run verify` → rc 0. Commit:
...
- [x] **Step 1: Write the failing tests** `tests/render/clouds.test.ts`:
...
- [x] **Step 3: Move the fog ramp.** In `horizon.ts` add:
...
- [x] **Step 4: Write `src/render/scene/clouds.ts`**:
...
- [x] **Step 5: Run** the clouds tests → PASS; the terrain tests unchanged. `npm run verify` → rc 0. Commit:
...
- [x] **Step 1: Load and create.** After `oceanDepth = await loadDepth()`:
...
- [x] **Step 2: Per frame.** Immediately before the `for (const cascade of cascades)` dispatch loop, compute the ocean time once and reuse it:
...
- [x] **Step 3: Diagnostics.** `diagnostics.ts`: add `readonly clouds: () => { readonly layers: readonly CloudLayer[]; readonly tier: CloudTierName | 'off'; readonly steps: number }` (import the two types). In the hook:
...
- [x] **Step 4: Typecheck, `npm run verify`** → rc 0. Then a first look on the reference GPU before any spec exists:
...
- [x] **Step 1: Write `tests/e2e/clouds.spec.ts`**:
...
- [x] **Step 2: Run it**, then read every screenshot. If the budget fails: lower `CLOUD_TIERS.high.cumulusSteps` 48 → 40 → 32, re-run, record each number in the ledger; if 32 still fails, stop and report -- the half-resolution contingency (design §7) is a new task, not an improvisation. If a screenshot shows the runway painted over or the airplane not occluding cloud, the depth path failed: stop and report with the screenshot.

- [x] **Step 3: Re-run the neighbours**: `deckQuals.spec.ts`, `gunnery.spec.ts`, `terrain.spec.ts` (its budget now includes the clouds over Leyte) and `title.spec.ts`. Read the deck-quals parked screenshot (clouds over the carrier) and confirm the gunnery ones are unchanged (clear range).

- [x] **Step 4: Docs.** README: a "Plan 16a" paragraph after the title-screen one, and `?cloudTier=` beside `?oceanTier=` in the Tier 2 section. §15: add row `| 16 | any | Clouds | §4 | 16a volumetric layers + whiteout landed 2026-09-19 ([design](2026-09-19-clouds-design.md), [handoff](../../handoff/2026-09-19-plan16a-clouds.md)); 16b cloud shadows and 16c sun angle to be designed |`. Handoff with every number, the screenshot descriptions, and which levers were pulled.

- [x] **Step 5: `npm run verify`** → rc 0. Commit:
